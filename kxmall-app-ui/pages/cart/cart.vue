	...
	export default {
		...
		methods: {
			...
			numberChange(data){
				...
				if(that.cartList[data.index].activityId !== 0 && that.cartList[data.index].couponId !== 0){
					this.$api.msg('抢购商品只允许购买一件')
					that.cartList[data.index].num = 1
					return
				}
				if(data.number > that.cartList[data.index].num){
				...
				}else if(data.number < that.cartList[data.index].num){
					var cartNum = this.$store.state.cartNum-1
					that.$store.commit('addCart',cartNum)
					uni.setTabBarBadge({
						index:2,
			...
			//清空失效商品
			clearStockLess(){
				that.$api.request('cart', 'removeCartItemBatch', {
			...
			calcTotal(){
				...
				list.forEach(item=>{
					...
					if(item.checked === true){
						...
						that.yuanjia += Number((item.originalPrice*item.num).toFixed(2))
