<template>
	<view class="container">
		...
		<view class="introduce-section">
			...
			<view class="flex align-start justify-between padding-top-sm">
				...
				<view class="flex">
					<view @click="toFavorite" style="text-align: center;">
						<image src="../../static/product/collect.png" mode="aspectFit"
			...
			<view class="flex align-center justify-between ">
				...
				<view style="font-size: 28rpx;color: #8D8E99;line-height: 40rpx;">
					{{goods.stockDto.sales}}人购买
				</view>
			</view>
			<!-- <view class="flex justify-between align-center">
				<view class="price-box align-center">
					<text class="price-tip">¥</text>
					<text class="price" style="font-size: 36rpx;">{{isVip ? (selectedSku.vipPrice ? selectedSku.vipPrice : goods.vipPrice) / 100.0  + ' [VIP]': (selectedSku.price ? selectedSku.price : goods.price) / 100.0 }}</text><text class="text-gray text-df" style="font-size: 24rpx;">/{{goods.unit}}</text>
					<text v-if="(isVip ? (selectedSku.vipPrice ? selectedSku.vipPrice : goods.vipPrice) : (selectedSku.price ? selectedSku.price : goods.price)) < (selectedSku.price ? selectedSku.originalPrice : goods.originalPrice)"
					 class="coupon-tip" style="margin-left: 12rpx;">{{parseInt((isVip? (selectedSku.vipPrice ? selectedSku.vipPrice : goods.vipPrice): (selectedSku.price ? selectedSku.price : goods.price)) / (selectedSku.originalPrice ? selectedSku.originalPrice : goods.originalPrice) * 100) / 10}}折</text>
				</view>
				<view style="background-color: #ffcf7c; color: #d89975;line-height: 1.5;" class="round text-sm padding-lr-sm">
		...
		<!-- 底部操作菜单 -->
		<!-- <view class="page-bottom" style="display: flex;justify-content: space-around;">
			...
			<view class="action-btn-group" style="margin-right: 20rpx;"> -->
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			that.$api.request('goods', 'getGoodsByStorage', {
				spuId: options.id,
